...
import ShowUserDetailsForEveryUser from './ShowUserDetailsForEveryUser';

const ShowSearchUserCard = (props) => {
            ...
            {users.length > 0 ?
                <Paper className="mx-3" elevation={3} sx={{ width: 620 }}>
                    {users.map((user, index) => (
                        <ShowUserDetailsForEveryUser key={index} userDetail={user} closeChattingModal={closeChattingModal} />
                    ))}
                </Paper>
                :
...
export default ShowSearchUserCard
